...
export default function Cards({ cards, gameType, socketID, clickHandler, userCanClick }) {
  ...
  return (
    <LayoutStyle>
      ...
      {gameType === 'VS Person' && (
        <>
          <ChatBox socketID={socketID} closeChatBoxFunc={() => setIsChatBoxOpen(false)} isOpen={isChatBoxOpen} setIsOpen={setIsChatBoxOpen} />
          <FooterStyle>
            <Icon text={'chat'} clickHandler={() => setIsChatBoxOpen((isChatBoxOpen) => !isChatBoxOpen)} />
          </FooterStyle>
        </>
      )}
    </LayoutStyle>
