    ...
    for (let rowIndex = 0; rowIndex < 8; rowIndex++) {
        for (let columnIndex = 0; columnIndex < 8; columnIndex++) {
            if (board[rowIndex][columnIndex].piece === color) {
                const legalMoves = getLegalMoves(board, [rowIndex, columnIndex], color, board[rowIndex][columnIndex].isKing);
    ...
    for (let rowIndex = 0; rowIndex < 8; rowIndex++) {
        for (let columnIndex = 0; columnIndex < 8; columnIndex++) {
            if (board[rowIndex][columnIndex].piece === 'black') {
                pieces[[rowIndex, columnIndex]] = getLegalMoves(board, [rowIndex, columnIndex], 'black', board[rowIndex][columnIndex].isKing);
            }
            if (pieces[[rowIndex, columnIndex]] && pieces[[rowIndex, columnIndex]].length === 0) delete pieces[[rowIndex, columnIndex]];
        }
    }
    ...
    for (let rowIndex = 0; rowIndex < 8; rowIndex++) {
        for (let columnIndex = 0; columnIndex < 8; columnIndex++) {
            if (board[rowIndex][columnIndex].piece === color) {
                const legalMoves = getLegalMoves(board, [rowIndex, columnIndex], color, board[rowIndex][columnIndex].isKing);
                for (const move of legalMoves) {
                    const newBoard = movePiece(JSON.parse(JSON.stringify(board)), rowIndex, columnIndex, color, board[rowIndex][columnIndex].isKing, move);
