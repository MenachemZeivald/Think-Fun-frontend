...
export default function GamePlay({ level, winner, setWinner }) {
  ...
  const movePieceMemoized = useCallback((board, rowIndex, columnIndex, color, isKing, nextIndex) => {
    ...
    if (level === 'person') {
      socket.on('active-game', (gameObgReceive) => {
        setBoard((board) => {
          ...
          return movePieceMemoized(board, rowIndex, columnIndex, color, isKing, [rowIndex2, columnIndex2]);
        });
  ...
  useEffect(() => {
    if (level !== 'person' && turn === 'black') {
      setTimeout(() => {
        ...
        let tempBoard = movePiece(board, pieceRow, pieceColumn, 'black', isKing, nextPosition);
        setBoard(tempBoard);
        setTurn('red');
      }, 1000);
    }
  ...
  function squareClickHandler(rowIndex, columnIndex, piece) {
    ...
    let [legalMoves, doubleJumpMoves] = getLegalMoves(board, [rowIndex, columnIndex], piece.piece, piece.isKing);
  ...
  function moveClickHandler(rowIndex, columnIndex) {
    ...
    let tempBoard = movePiece(board, chosenPieceRow, chosenPieceColumn, color, isKing, [rowIndex, columnIndex]);
    setBoard(tempBoard);
    setChosenPiece(null);
    setLegalMoves([]);
