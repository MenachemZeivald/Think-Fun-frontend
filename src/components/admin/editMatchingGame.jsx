...
export default function EditMatchingGame() {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const matchingGameInit = async () => {
		try {
			let url = '/games/matchingGame/single/' + params['id'];
	...
	const editmatchingGame = async bodyData => {
		try {
			let url = '/games/matchingGame/' + params['id'];
				...
				<div className='form-container'>
					{!info._id ? (
						...
						<form
							...
							id='id_form'
							className='col-md-12 shadow p-2'
						>
							...
							<label>img_url</label>
							<input
								{...register('img_url', { minLength: 2, required: true })}
								className='form-control '
								type='text'
