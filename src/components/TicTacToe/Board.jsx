...
export default function Board({ board, makeTurn, socketID, myTurn, winArr = [], resetFunc, vsPerson, userSign, setBoard }) {
    ...
    return (
        <LayoutStyle>
            <BoardStyle>
                {board.map((place, i) => {
                    return <Square key={i} place={place} index={i} clickable={myTurn && !winArr.length} clickHandler={makeTurn} findInWinArr={winArr.includes(i)}></Square>;
                })}
            </BoardStyle>
